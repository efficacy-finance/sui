...
impl RWLockDependencyBuilder {
    ...
    pub fn add_dependencies_for(
        &self,
        digest: TransactionDigest,
        v: &mut HashSet<TransactionDigest>,
    ) {
...
mod tests {
    ...
    pub fn test_casual_order_rw_locks() {
        ...
        let r = extract(CasualOrder::casual_sort(vec![e1, e2, e3]));
        assert_eq!(r.len(), 3);
        assert_eq!(*r.get(2).unwrap(), 3); // [3] is the last
                                           // both [1] and [2] are present (but order is not fixed)
        assert!(r.contains(&1));
        assert!(r.contains(&2));
    }
